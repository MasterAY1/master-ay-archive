...
import { 
  ...
  User, ChevronDown, Check, Plus, MoreHorizontal, PenSquare
} from 'lucide-react';

// --- ADVANCED MOCK HISTORY DATA ---
const CHAT_HISTORY = {
  today: [
    { id: 1, title: "Next.js Authentication Flow" },
    { id: 2, title: "Optimize WebGL rendering" }
  ],
  yesterday: [
    { id: 3, title: "Draft freelance contract" },
    { id: 4, title: "Python FastAPI deployment" }
  ],
  previous: [
    { id: 5, title: "Tailwind grid vs flexbox" },
    { id: 6, title: "PostgreSQL vs SQLite" }
  ]
};
...
export default function NovaAI() {
  ...
  const [messages, setMessages] = useState([
    { 
      id: 1, 
      role: 'ai', 
      content: "Hello, Architect. I am Nova. How can I assist you with your codebase or strategy today?" 
    }
  ]);
      ...
      <div className={`${sidebarOpen ? 'w-[260px] opacity-100' : 'w-0 opacity-0'} bg-[#0D0D0D] transition-all duration-300 ease-in-out flex flex-col shrink-0 overflow-hidden relative z-20`}>
        ...
        <div className="p-3 sticky top-0 bg-[#0D0D0D] z-10">
          <button 
            ...
            className="w-full flex items-center justify-between p-3 rounded-lg hover:bg-[#212121] transition-colors text-sm font-medium group"
          >
            <div className="flex items-center gap-3">
              <div className="w-7 h-7 rounded-full bg-[#ECECEC] text-[#0D0D0D] flex items-center justify-center">
                <Sparkles size={14} />
              </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar px-3 pb-4">
          ...
          <div className="mt-4 mb-2 px-3 text-xs font-semibold text-[#808080]">Today</div>
          ...
          <div className="mt-6 mb-2 px-3 text-xs font-semibold text-[#808080]">Yesterday</div>
          ...
          <div className="mt-6 mb-2 px-3 text-xs font-semibold text-[#808080]">Previous 7 Days</div>
          {CHAT_HISTORY.previous.map(chat => (
             <button key={chat.id} className="w-full text-left p-2.5 px-3 rounded-lg hover:bg-[#212121] transition-colors text-sm text-[#CCCCCC] hover:text-[#ECECEC] truncate group relative">
              {chat.title}
             <div className="absolute right-2 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 bg-gradient-to-l from-[#212121] via-[#212121] pl-4">
               <MoreHorizontal size={16} className="text-[#808080] hover:text-white" />
      ...
      <div className="flex-1 flex flex-col h-screen relative bg-[#171717]">
        ...
        <header className="h-14 flex items-center justify-between px-4 sticky top-0 z-10 bg-[#171717]/80 backdrop-blur-md">
          <div className="flex items-center gap-2">
            <button 
              ...
              className="p-2 text-[#808080] hover:text-[#ECECEC] rounded-lg hover:bg-[#212121] transition-colors"
            >
              {sidebarOpen ? <PanelLeftClose size={20} /> : <PanelLeft size={20} />}
            </button>
            
            <button className="flex items-center gap-2 px-3 py-1.5 rounded-lg hover:bg-[#212121] transition-colors group">
              <span className="text-lg font-semibold text-[#ECECEC] tracking-tight">Nova <span className="text-[#808080] font-normal">3.1</span></span>
          ...
          <div className="max-w-3xl mx-auto py-8 space-y-12 pb-40">
            {messages.map((msg) => (
              <div key={msg.id} className={`flex w-full ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                /* AI Message Style (Borderless, Gemini/ChatGPT feel) */
                  <div className="flex gap-5 w-full max-w-3xl">
                    ...
                    <div className="flex-1 space-y-4 min-w-0">
                      <div className="text-[15px] leading-[1.7] text-[#D1D5DB] font-normal">
                        {msg.content.split('\n').map((line, i) => (
                          <p key={i} className={line.trim() === '' ? 'h-4' : 'mb-4 last:mb-0'}>
                            {line}
                          </p>
                        ))}
                      </div>
                      ...
                      <div className="flex items-center gap-1 -ml-2">
                        <button 
                          ...
                          className="p-2 text-[#808080] hover:text-[#ECECEC] hover:bg-[#212121] rounded-lg transition-colors flex items-center gap-1.5"
                        >
        ...
        <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-[#171717] via-[#171717] to-transparent pt-10 pb-6 px-4 md:px-0">
          <div className="max-w-3xl mx-auto relative">
            <form 
              onSubmit={handleSend}
              className={`relative bg-[#212121] border border-white/5 rounded-2xl overflow-hidden shadow-[0_0_15px_rgba(0,0,0,0.1)] transition-all duration-200 focus-within:border-white/20`}
            >
              <textarea 
                ref={textareaRef}
                value={prompt}
                ...
                className="w-full bg-transparent px-4 py-4 pr-14 text-[15px] text-[#ECECEC] resize-none focus:outline-none min-h-[56px] max-h-[200px] custom-scrollbar placeholder-[#808080]"
                rows="1"
              />
            ...
            <div className="text-center mt-3 text-xs text-[#808080]">
